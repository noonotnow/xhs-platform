...
function hasLocalBrowserTestAccess(request: NextRequest) {
  const expected = process.env.XHS_BROWSER_TEST_COOKIE;
  return process.env.NODE_ENV !== 'production'
    && Boolean(expected)
    && request.cookies.get('__xhs_browser_test')?.value === expected;
}

export async function middleware(request: NextRequest) {
  if (isMachineApi(request.nextUrl.pathname) || hasLocalBrowserTestAccess(request)) {
