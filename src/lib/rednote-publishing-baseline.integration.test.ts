...
describe('canonical local publishing migration chain', () => {
  ...
  it('can reapply migration 032 without mutating recovery audit rows', async () => {
    const migration = await readFile(
      path.join(
        process.cwd(),
        'migrations',
  ...
  it('can reapply migration 034 without mutating recovery audit rows', async () => {
    const migration = await readFile(
      path.join(
        process.cwd(),
        'migrations',
  ...
  it('can reapply migration 035 without mutating recovery audit rows', async () => {
    const migration = await readFile(
      path.join(
        process.cwd(),
        'migrations',
  ...
  it('selects migration 030 when the recovery blocker signature is missing', async () => {
    ...
    try {
      for (const file of migrationFiles.slice(
        0,
        migrationFiles.indexOf('030_revision_aware_publish_lifecycle.sql'),
      )) {
        const migration = await readFile(
